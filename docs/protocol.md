...
- 状态：D-013～D-018、D-021～D-030 已批准的协议与状态存储基线；不是已发布 API，最终 HTTP 路径仍需在 `chalsense-server` 创建前评审。
...
标签含义沿用 `docs/threat-model.md`。D-001～D-030 均作为已批准前提；文中“建议”只表示尚未冻结为公开 HTTP 表面的实现细节，不得改变已批准安全语义。
