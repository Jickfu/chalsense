...
- 日期：2026-08-23
- 状态：已批准；只批准采集与评估方法，不冻结最终 tolerance 或 `policyVersion`。
- 结论：真实用户校准必须使用独立、明确同意、本地优先的研究工具，不接入生产 Widget 默认路径或 HTTP 协议。工具不得联网或持久存储，不采集完整轨迹、IP、UA、业务/账号字段、时间戳、设备指纹或用户/会话标识；只导出落点绝对误差、耗时、输入类型、显示宽度和 DPR 的独立粗粒度聚合。
- 结论：首轮只招募明确同意的成年人，每人最多 10 次；建议至少 20 名参与者、100 次有效尝试，公开任一分组必须 `n >= 20`。单人聚合最长保留 90 天并与提交身份元数据分离，形成合并直方图后删除；无标识合并结果可随研究报告保留。
- 结论：分桶结果只报告 candidate tolerance 的确定接受/可能接受上下界，不插值制造精度。真实用户数据不得变成行为识别或攻击训练轨迹；最终 tolerance 必须与独立攻击回归共同评审，再以新决策冻结。
- 原因：容差需要真实交互证据，但完整轨迹和设备组合具有行为与指纹风险；本地边际聚合以更小隐私面回答落点误差问题，并保留 Q-004 的安全权衡门禁。

### D-041 安全漏洞披露渠道

- 日期：2026-08-23
- 状态：已批准。
- 结论：公开仓库使用根目录 `SECURITY.md` 与 GitHub Private Vulnerability Reporting 作为唯一推荐的敏感漏洞入口；未修复利用细节、有效 secret/ticket、个人数据和生产样本不得提交公开 Issue/Discussion/PR。
- 结论：以 3 个工作日确认、7 个工作日初步分类为尽力响应目标，不构成 SLA。披露日期、修复和署名与报告者协调；安全港只覆盖获授权、最小影响、避免隐私侵害和拒绝服务的善意研究。
- 原因：提供不依赖公开泄露或个人邮箱的结构化私密渠道，同时诚实反映当前未发布 `0.x` 项目没有生产支持承诺。
